...
export default class FmCommand extends SlashCommand {
    ...
    public data = new SlashCommandBuilder()
        ...
        .addSubcommand((subcommand) =>
            subcommand.setName('unset').setDescription('Unlink your Last.fm account'),
        )
        .addSubcommand((subcommand) =>
            subcommand
                .setName('np')
                ...
                .addUserOption((option) =>
                    option.setName('user').setDescription('User to check (defaults to you)'),
                ),
        )
        .addSubcommand((subcommand) =>
            subcommand
    ...
    private async handleSet(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        try {
            ...
            await interaction.editReply(
                `Successfully linked your Last.fm account: **${username}**\n` +
                    `You can now use other /fm commands!`,
            );
        } catch (error) {
            await interaction.editReply(
                `Failed to link Last.fm account. Please make sure the username **${username}** exists on Last.fm.`,
            );
        }
    }

    private async handleUnset(interaction: ChatInputCommandInteraction): Promise<void> {
        const userId = interaction.user.id;
    ...
    private async handleNowPlaying(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        if (tracks.length === 0) {
            await interaction.editReply(
                `${targetUser.id === interaction.user.id ? 'You have' : `${targetUser.username} has`} not listened to anything recently.`,
            );
            return;
        }
    ...
    private async handleTop(interaction: ChatInputCommandInteraction): Promise<void> {
        const type = interaction.options.getString('type', true) as 'artists' | 'albums' | 'tracks';
        const period = (interaction.options.getString('period') || 'overall') as LastFmPeriod;
        const targetUser = interaction.options.getUser('user') || interaction.user;
        await interaction.deferReply();
        ...
        if (type === 'artists') {
            ...
            embed.setDescription(
                albums
                    .map(
                        (album, i) =>
                            `${i + 1}. **${album.artist}** — ${album.name} (${album.playcount} plays)`,
                    )
                    .join('\n'),
            );
            if (albums[0].image) {
            ...
            embed.setDescription(
                tracks
                    .map(
                        (track, i) =>
                            `${i + 1}. **${track.artist}** — ${track.name} (${track.playcount} plays)`,
                    )
                    .join('\n'),
            );
            if (tracks[0].image) {
     ...
     */
    private async getUsername(user: User): Promise<string> {
        const username = await this.lastFmService.getUsername(user.id);
        if (!username) {
            throw new Error(
                `${user.id === user.client.user!.id ? 'You do' : `${user.username} does`} not have a linked Last.fm account. ` +
