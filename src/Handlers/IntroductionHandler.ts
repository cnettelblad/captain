...
export default class IntroductionHandler extends Handler<'messageCreate'> {
    private introductionChannels: string[] = [
        '1370421583683915788', // Test Server
        '590966486567354370' // Wanderlust
    ]

    private newIntroductionMessages = [
    ...
    private oldIntroductionMessages = [
    ...
    async handle() {
        ...
        if (!this.introductionChannels.includes(message.channel.id)) return;
        ...
        message.startThread({
            ...
            thread.send(welcomeMessage.replace(':user', `<@${message.author.id}>`)).catch(console.error);
        }).catch(console.error);
